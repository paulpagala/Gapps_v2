...
export default function ParkingDashboard() {
  ...
  const parkingArea = JSON.parse(window.localStorage.getItem('parkingArea'))
  const parkingAddress = JSON.parse(window.localStorage.getItem('parkingAddress'))
  const rows = [
    createData(parkingArea, parkingAddress, 'Active', 'pj'),
    // createData('The Globe Tower', 'Not Specified', 'Active','pj'),
    // createData('Three Parkade', 'H23X+5HH, Taguig, Metro Manila','Active','pj'),
  ];

  const weekday = React.useState(window.localStorage.getItem('weekday'))
  const serviceFee = React.useState(JSON.parse(window.localStorage.getItem('serviceFee')))
  const earliestBook = React.useState(JSON.parse(window.localStorage.getItem('earliestBook')))
  const cancelDeadline = React.useState(JSON.parse(window.localStorage.getItem('cancelDeadline')))
      ...
      <TabPanel value={value} index={1}>
        ...
        <Paper variant="outlined" sx={{ ml: '4%', my: { md: 3, lg: 5 }, p: { md: 2, lg: 3 }, backgroundColor: '#FAFAFA', width: '55%' }}>
          ...
          <Box>
            ...
            <Typography>{earliestBook} days before the booking</Typography>
          </Box>
          <Box>
            <Typography sx={{ fontWeight: 'bold' }}>Cancellation deadline</Typography>
            <Typography>{cancelDeadline} minutes before the booking</Typography>
          </Box>
          <Box>
